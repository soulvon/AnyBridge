...
| `ui-src/partials/scripts-app.html` | classic script 加载顺序 |
...
<!-- 传 this / value / checked / event -->
<input type="checkbox" data-action="toggleCbModelEnabled" data-events="change" data-args="[0]" data-pass-checked>

<!-- 多事件不同处理 -->
<div data-actions='{"click":{"action":"toggleEvalCombo","args":["eval-provider-select"],"passEvent":true},"keydown":{"action":"handleEvalComboTriggerKey","args":["eval-provider-select"],"passEvent":true}}'></div>

<!-- 动态模板里的调用串（仅简单 fn('a', true)） -->
...
所有 JS 仍使用普通 classic script，而不是 `type="module"`，因为业务函数仍挂在全局供 `data-action` 解析。新增拆分文件时必须在 `ui-src/partials/scripts-app.html` 中保持依赖顺序：bridge → actions → shell/runtime → feature → updater → init。
...
3. 把 classic scripts 改成 `type="module"`，用显式 import/export 管依赖。  
4. 模块化后再抽共享层：
