...
impl App {
    pub async fn new(mut config: Config) -> Result<Self> {
        ...
        if let Some(routes_path) = config.anybridge_routes_path.as_deref() {
            crate::anybridge::sync_routes_to_store(
                &store,
                routes_path,
                config.anybridge_cursor_models_path.as_deref(),
                config.anybridge_gateway_url.as_deref().unwrap_or_default(),
                config.anybridge_gateway_key.as_deref().unwrap_or_default(),
            )
            .await?;
        }
