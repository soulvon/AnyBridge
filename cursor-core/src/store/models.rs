...
impl Store {
    ...
    pub async fn replace_models(&self, inputs: &[ModelConfigInput]) -> Result<Vec<ModelConfig>> {
        let mut normalized = Vec::with_capacity(inputs.len());
        let mut hashes = HashSet::with_capacity(inputs.len());
        for input in inputs {
            let input = normalize_model_input(input)?;
        ...
        let _write = self.writes.lock().await;
        let mut transaction = self.pool.begin().await?;
        sqlx::query("UPDATE llm_calls SET model_hash = NULL WHERE model_hash IS NOT NULL")
            .execute(&mut *transaction)
            .await?;
