...
impl CaManager {
    pub fn managed() -> Result<Self> {
        let dir = std::env::var_os("ANYBRIDGE_CONFIG_DIR")
            .map(PathBuf::from)
            .map(|root| root.join("certs"))
            .unwrap_or(managed_data_dir()?.join("ca"));
        Ok(Self { dir })
    }

    fn cert_path(&self) -> PathBuf {
        if self.dir.ends_with("certs") {
            self.dir.join("server.codeium.com.pem")
        } else {
    ...
    fn key_path(&self) -> PathBuf {
        if self.dir.ends_with("certs") {
            self.dir.join("server.codeium.com-key.pem")
        } else {
