        ...
        Box::pin(try_stream! {
            ...
            let (recorder, _cancel_on_drop, mut stream): (CallRecorder, CancelOnDrop, ProviderStream) =
                if selected.starts_with(ADAPTER_ID_PREFIX) {
                    ...
                    let guard = recorder.cancel_on_drop();
                    let api_key = if model.api_key == crate::anybridge::ANYBRIDGE_GATEWAY_KEY_SENTINEL {
                        std::env::var("ANYBRIDGE_LOCAL_PROXY_KEY").map_err(|_| {
                            Error::Config("AnyBridge local gateway key is unavailable".into())
                        })?
                    } else {
                        model.api_key.clone()
                    };
