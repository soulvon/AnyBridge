...
```
AnyBridge = Tauri 2.x (Rust 壳) + Node.js Sidecar (pkg 打包 EXE) + 前端 UI (单文件 HTML)
...
│  └─ sidecar: anybridge-proxy.exe (pkg Node.js)   │
...
| 5 | **配置明文存储** | 🟠 中 | `%APPDATA%\anybridge\byok-config.json` 和 `providers.json` 明文可读 |
...
| 保护措施 | WindsurfGate (未破) | Infinity (已破) | **AnyBridge (当前)** |
|----------|-------------------|----------------|-------------------|
| Rust 后端 | ✅ | ✅ | ✅ |
| 反调试 | ✅ 7种方法 | ❌ | ❌ |
| 代码完整性 | ✅ 代码段校验 | ❌ | ❌ |
...
打包时生成哈希：
```bash
sha256sum src-tauri/binaries/anybridge-proxy-x86_64-pc-windows-msvc.exe \
...
fn derive_encryption_key() -> [u8; 32] {
    ...
    let app_id = "com.anybridge.desktop";
    ...
    let mut mac = HmacSha256::new_from_slice(b"anybridge-config-v1").unwrap();
